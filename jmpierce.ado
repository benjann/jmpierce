*! version 1.0.9  09mar2023  Ben Jann
...
program define jmpierce, rclass sortpreserve
    ...
    if `"`blocks'"'!="" {
        di as err "blocks() option discontinued"
        exit 198
    }
...
//replay reference estimates: compute linear prediction
        if "`estp'"=="" { //use average coefficients
            ...
            if `"`blocks'"'!="" {
                tempname Bref
                mat `Bref' = e(b)
                local firsteq: coleq `Bref', quoted
                local firsteq: word 1 of `firsteq'
                mat `Bref' = `Bref'[1,"`firsteq':"]
            }
            tempvar y1
...
//compute average cdf of residuals
    tempvar rref cdfref refgrp
    qui gen byte `refgrp' = 0
    if "`estp'"=="`est1'" {
        qui gen `rref' = `r' if `g1'
        qui gen `cdfref' = `cdf' if `g1'
        qui replace `refgrp' = 1 if `g1'
    }
    else if "`estp'"=="`est2'" {
        qui gen `rref' = `r' if `g2'
        qui gen `cdfref' = `cdf' if `g2'
        qui replace `refgrp' = 1 if `g2'
    }
    else {
        qui gen `rref' = `r' if ( `g1' | `g2' )
        cumul `r' [`weight1'`wexp1'] if ( `g1' | `g2' ) , g(`cdfref') equal
    }

//compute hypothetical residuals
    tempvar rhyp
    invcdf `cdf' if `refgrp'==0 & ( `g1' | `g2' ), ref(`rref') cdf(`cdfref') g(`rhyp')
    qui replace `rhyp' = `r' if `refgrp'==1
...
//calculate quantity effect for groups of variables
    if `"`blocks'"'!="" {
        tempname blk
        BlockWise `blk' `Bref' `rref' `cdf' `refgrp' `g1' "`weight1'`wexp1'" `g2' "`weight2'`wexp2'", ///
...
program define Stats
    local statslist mean sd variance p5 p10 p25 p50 median p75 p90 p95 /*
    ...
    foreach st of local 0 {
        ...
        else if substr("variance",1,max(3,length("`st'")))=="`st'" local st variance
...
program define BlockWise
    ...
    args blk Bref rhyp g1 weight1 g2 weight2
    local vars: colnames `Bref'
    ...
    if `nblocks'==0 c_local blocks
        ...
        mat rown `blk' = `stats'
        mat coln `blk' = `bnames'
        if colnumb(`Bref',"_cons")<. { // this is for precission reasons only
            mat `btmp' = `Bref'[1,"_cons"]
        }
*       forv j=1/`nblocks' {
*           local vars: word `j' of `bvars' // will break if length(vars)>503
        ...
        foreach vars of local bvars {
            local ++j
            foreach var of local vars {
                mat `btmp' = nullmat(`btmp'), `Bref'[1,"`var'"]
            }
